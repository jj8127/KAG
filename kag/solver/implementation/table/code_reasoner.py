...
from kag.solver.implementation.table.python_coder_with_thinking import PythonCoderAgentWithThinking as PythonCoderAgent

logger = logging.getLogger()


class CodeReasoner(KagReasonerABC):
    """
    table reasoner
    """
    ...
    def __init__(
        ...
        self.llm_retrieval_prompt = PromptOp.load(
            self.biz_scene, "llm_retrieval_prompt_with_code"
    ...
    def reason(self, question: str, context:str):
        ...
        if question.startswith(CodeReasoner.DOMAIN_KNOWLEDGE_INJECTION):
            self._save_dk(question, history)
            return "done"
        elif question.startswith(CodeReasoner.DOMAIN_KNOWLEDGE_QUERY):
        ...
        while try_times > 0:
            ...
            for sub_question in sub_question_list:
                sub_question_str += sub_question["sub_question"] + "\n"
            
            # 获取整体code
            code_agent = PythonCoderAgent(question, question, history, context, **self.kwargs)
            node = SearchTreeNode(question, "PythonCoder")
            history.add_now_procesing_ndoe(node)
            code = self._call_python_coder_func(
                        code_agent=code_agent, node=node, thinking=sub_question_str
                    )

            # 拆分需要input的问题
            retrieval_questions = self._get_retrieval_questions(code)
            for sub_question in retrieval_questions:
                sub_q_str = sub_question
                func_str = "Retrieval"
                ...
                retrieval_context.append(sub_answer)
        ...
        return final_answer, history, sub_question_list, str(retrieval_context), code
    ...
    def _get_llm_retrieval(self, question: str,node: SearchTreeNode, history: SearchTree, context: str):
        ...
        sub_answer = str(sub_answer).strip()
    ...
    def _get_retrieval_questions(self, code: str):
        questions = []
        lines = code.split("\n")
        for line in lines:
            if "input" in line:
    ...
    def _call_python_coder_func(
        self, code_agent: PythonCoderAgent, node: SearchTreeNode, thinking: str
    ):
        code = code_agent.get_code(thinking = thinking)
        node.answer = code
        node.answer_desc = self._process_coder_desc(code)
        return code
    
    def _run_python_coder_func(
        self, code_agent: PythonCoderAgent, node: SearchTreeNode, parameters: list, retrieval_questions: list
    ):
        rst, run_error, code = code_agent._run_onetime(parameters = parameters)
        rst = self._process_coder_out(rst, retrieval_questions, parameters)
        node.answer = rst
        return rst, run_error, code 

    def _process_coder_out(self, rst: str, retrieval_questions:list, parameters: list): 
        for question, parameter in zip(retrieval_questions, parameters):
            rst = rst.replace(question, question +': '+ parameter+'\n')
        return rst
    ...
    def _save_dk(self, dk: str, history: SearchTree):
        dk = dk.strip(CodeReasoner.DOMAIN_KNOWLEDGE_INJECTION)
        dk = dk.strip().strip("\n")
